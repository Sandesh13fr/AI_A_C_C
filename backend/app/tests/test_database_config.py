...
def test_railway_public_postgres_urls_are_normalized() -> None:
    settings = Settings(
...
def test_railway_internal_postgres_urls_do_not_force_ssl() -> None:
    settings = Settings(
        database_url="postgres://postgres:secret@postgres.railway.internal:5432/railway?sslmode=require",
        sync_database_url="",
    )

    assert settings.database_url == "postgresql+asyncpg://postgres:secret@postgres.railway.internal:5432/railway"
    assert settings.sync_database_url == "postgresql://postgres:secret@postgres.railway.internal:5432/railway"


def test_railway_public_postgres_connect_args_enable_ssl() -> None:
...
def test_railway_internal_postgres_connect_args_skip_ssl() -> None:
    database_url = "postgresql+asyncpg://postgres:secret@postgres.railway.internal:5432/railway"

    assert _async_connect_args(database_url) == {}
    assert _sync_connect_args(database_url) == {}
